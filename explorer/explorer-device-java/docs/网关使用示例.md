...
示例中编辑 [IoTGateway.java](../src/test/java/IoTGateway.java) 文件中的参数配置信息
```
{
...
```
如果控制台创建设备使用的是密钥认证方式，需要在 MqttSample.java 填写 mProductID（产品ID）、mDevName（设备名称）、mDevPSK（设备密钥），示例中使用的是密钥认证。

#### 证书认证接入

将证书和私钥放到 [resources](../src/test/resources/)文件夹中，将psk设置null，然后将证书名称填写入指定区域

如果控制台创建设备使用的是证书认证方式，除了需要在 MqttSample.java 填写 mProductID（产品ID）、mDevName（设备名称），mDevPSK（设备密钥）设置为null，还需修改 mDevCert (设备证书文件名称)、mDevPriv(设备私钥文件名称)
```
    private static String mDevCert = "";           // 填写 resources 文件夹下设备证书文件名称
    private static String mDevPriv = "";           // 填写 resources 文件夹下设备私钥文件名称
```
