...
* 将qcloud-iot-explorer-sdk-java项目导入
* 基于API进行开发
- SDK 依赖开源社区实现的 mqtt 协议**eclipse.paho**，因此需在 pom.xml中增加 eclipse.paho 的依赖：
...
![](res\ea345acb67bd0f9ef20a7336704bd070.jpg)
...
了解数据模板基本功能，可参考`IoTDataTemplate.java`和`DataTemplateSample.java`。

直连设备开发可参考`IoTLight.java`和`LightSample.java`。

网关设备和子设备开发可参考`IotGateway.java`、`GatewaySample.java`、`ProductLight.java`和`ProductAirconditioner.java`。