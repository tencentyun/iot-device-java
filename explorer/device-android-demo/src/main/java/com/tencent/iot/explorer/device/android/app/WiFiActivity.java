...
public class WiFiActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void createWifiHotspot() {
        ...
        boolean result = wiFiHostpotUtil.createWifiHotspot(WIFI_HOTSPOT_SSID, password);
        if (result) {
                        ...
                        public void run(){
                            WifiUtils.connectWifiApByNameAndPwd(getApplicationContext(), ssid, password, new WifiUtils.WifiConnectCallBack() {
                                ...
                                public void connectResult(boolean connectResult) {
                                    ...
                                    if (!connectResult) {
                                        Log.d(TAG, "WifiUtils connectResult: 连接失败" );
                                    } else {
            ...
            textview.setText("热点已开启 SSID:" + WIFI_HOTSPOT_SSID + " password:" + password);
        } else {
...
//            options.setSocketFactory(AsymcSslUtils.getSocketFactory());   如果您使用的是3.3.0及以下版本的 explorer-device-rtc sdk，由于密钥认证默认配置的ssl://的url，请添加此句setSocketFactory配置。
    ...
    private void closeWifiHotspot() {
        ...
        wiFiHostpotUtil.closeWifiHotspot();
    }
