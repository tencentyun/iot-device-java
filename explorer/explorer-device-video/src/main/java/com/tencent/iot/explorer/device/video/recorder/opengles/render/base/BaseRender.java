...
import com.tencent.iot.explorer.device.video.recorder.utils.OpenGLESUtils;
import java.nio.FloatBuffer;

public class BaseRender implements IRender {

    private Context context;
    private BaseRenderBean renderBean; // 渲染数据
    private FloatBuffer vertexBuffer; // 顶点坐标
    private FloatBuffer coordinateBuffer; // 纹理坐标
    private int vertexSize = 2; // 顶点坐标维数（即 x, y, z）
    private int coordinateSize = 2; // 纹理坐标维数（即 x, y, z）
    private int vertexStride = vertexSize * 4; // 顶点坐标步长（即维数 * 字节数）
    private int coordinateStride = coordinateSize * 4; // 纹理坐标步长（即维数 * 字节数）
    private int vertexCount = 4; // 顶点个数
    private int coordinateCount = 4; // 纹理点个数
    private int vertexShader; // vertex shader
    private int fragShader; // frag shader
    private int program;
    private int textureId; // 纹理 id
    private int fboTextureId; // fbo 纹理 id
    private int fboId;
    private int vboId;
    private String vertexFilename; // 顶点着色器代码路径
    private String fragFilename; // 片元着色器代码路径
    private int width; // 尺寸 宽
    private int height; // 尺寸 高
    private boolean isBindFbo = false; // 是否绑定 Fbo
    private int aPosLocation; // 着色器顶点坐标位置
    private int aCoordinateLocation; // 着色器纹理坐标位置
    private int uSamplerLocation; // 着色器纹理位置
    private boolean isCreate = false; // 是否执行了 onCreate
    private boolean isChange = false; // 是否执行了 onChange
