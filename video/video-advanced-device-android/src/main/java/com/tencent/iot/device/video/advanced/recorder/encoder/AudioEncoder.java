package com.tencent.iot.device.video.advanced.recorder.encoder;
...
import com.tencent.iot.device.video.advanced.recorder.consts.LogConst;
...
import com.tencent.iot.device.video.advanced.recorder.listener.OnReadAECProcessedPcmListener;
...
public class AudioEncoder {
    ...
    private void initAudio() {
        ...
        try {
            ...
            Log.i(LogConst.RTC_TAG, "audioCodec MediaCodec createEncoderByType");
    ...
    private void addADTStoPacket(ByteBuffer outputBuffer) {
        ...
        if (encodeListener != null) {
            Log.i(LogConst.RTC_TAG, "on audio encoded byte: "+byte2HexOnlyLatest8(dataBytes) + "; seq: " + seq);
    ...
    class CodecThread extends Thread {
        ...
        public void run() {
            ...
            Log.i(LogConst.RTC_TAG, "audioRecord startRecording");
            audioCodec.start();
            Log.i(LogConst.RTC_TAG, String.format("audioCodec start with MediaFormat AudioSource: %d, SampleRateInHz: %d, IsChannelMono: %b, bitDepth: %d, encodeMime: %s, bitRate: %d, CodecProfileLevel: %d, KEY_MAX_INPUT_SIZE:%d",
            ...
            while (true) {
                ...
                if (audioInputBufferId >= 0) {
                    ...
                    if (readSize >= 0) {
                        inputBuffer.clear();
                        if (mAECProcessedPcmListener != null) {
                            Log.i(LogConst.RTC_TAG, String.format("audioRecord read capture original frame data before other process:%s, seq:%d", byte2HexOnlyLatest8(audioRecordData), beforSeq));
                            byte[] cancell = mAECProcessedPcmListener.onReadAECProcessedPcmListener(audioRecordData);
                            Log.i(LogConst.RTC_TAG, String.format("audioRecord read capture original frame data after other process:%s, seq:%d", byte2HexOnlyLatest8(audioRecordData), beforSeq));
                            beforSeq++;
                            inputBuffer.put(cancell);
                        } else {
                            inputBuffer.put(audioRecordData);
                            Log.i(LogConst.RTC_TAG, String.format("audioRecord read capture origina l frame data:%s", byte2HexOnlyLatest8(audioRecordData)));
                ...
                while (audioOutputBufferId >= 0) {
                    ...
                    if (audioInfo.size > 2) {
                        ...
                        Log.i(LogConst.RTC_TAG, String.format("audioCodec getOutputBuffer audioInfo.offset :%d + audioInfo.size :%d", audioInfo.offset, audioInfo.size));
