...
## 运行示例程序进行动态注册
运行示例程序前，需要配置 [TestDynregDevSample.java](../src/test/java/com/tencent/iot/hub/device/java/core/dynreg/TestDynregDevSample.java) 文件中 mProductID（产品ID）、mDevName（设备名称）参数外，还需要填写mProductKey（控制台中ProductSecret）

运行 [TestDynregDevSample.java](../src/test/java/com/tencent/iot/hub/device/java/core/dynreg/TestDynregDevSample.java) 的main函数，调用dynReg()，调用动态注册认证，动态注册回调获取了对应设备的密钥或证书相关信息后，再调用 MQTT 上线。示例代码如下：
```
...
```
24/02/2021 23:46:00,972 [main] DEBUG TestDynregDevSample dynReg 43  - Dynamic Register OK!
24/02/2021 23:46:01,209 [Thread-0] INFO  TestDynregDevSample onGetDevicePSK 454  - Dynamic register OK! onGetDevicePSK, devicePSK[**********************]//表示设备是通过密钥认证的
24/02/2021 23:46:01,209 [Thread-0] INFO  TestDynregDevSample onGetDevicePSK 454  - Dynamic register OK!onGetDeviceCert, deviceCert[**********************] devicePriv[**********************] //表示设备是通过证书认证的
```
