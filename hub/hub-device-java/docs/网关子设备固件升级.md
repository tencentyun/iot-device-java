...
请在 [GatewaySampleTest.java](../../hub-device-java/src/test/java/com/tencent/iot/hub/device/java/core/gateway/GatewaySampleTest.java) 文件中填入所需的参数。
```
...
运行 [GatewaySampleTest.java](../../hub-device-java/src/test/java/com/tencent/iot/hub/device/java/core/gateway/GatewaySampleTest.java) ，将 网关设备 进行 MQTT 认证连接，认证连接后，初始化OTA并使网关设备和子设备都订阅 Topic `$ota/update/${productId}/${deviceName}`。示例代码如下：
