...
import com.tencent.cloud.ai.fr.pipeline.Task;
...
public class Case {
     ...
     */
    public static class CaseSaveThread extends AbsPipelineThread<Case> {

        private DiskCache mDiskCache;
        private final String cacheDirPath;

        public CaseSaveThread(Context context) {
            super(CaseSaveThread.class.getSimpleName());
            cacheDirPath = String.format("/sdcard/%s/SavedCases/", context.getPackageName());

            ArrayList<Task<Case>> pipeline = new ArrayList<>();
            pipeline.add(new Task<Case>() {
                ...
                @Override
                public boolean onProcess(Case aCase) {
                    mDiskCache.saveCase(aCase);
                    return false;//没有下一个步骤了
                }
            });
            switchToPipeline(pipeline);
        }

        @Override
        protected void onThreadStart() {
            try {
                mDiskCache = new DiskCache(cacheDirPath);
            } catch (IOException e) {
                throw new RuntimeException("创建磁盘缓存失败: " + cacheDirPath, e);
            }
        }
        ...
        @Override
        protected void onRecycleWorkingStuff(Case stuff) {/*nothing*/}
    ...
    private static class DiskCache {
        ...
        private void save(Case c, Frame frame, String frameType) {
            ...
            save(c.frameGroup.colorFrame.data, baseFileName + FRAME_TYPE_COLOR);//原始图片
