...
public class YTSDKManager {
    ...
    public YTFaceRetrieval mYTFaceRetrieve;
    ...
    public YTSDKManager(AssetManager assetManager) {
        ...
        mYTFaceRetrieve = new YTFaceRetrieval(cvtTable, FACE_FEAT_LENGTH);
    }
    ...
    public float[] extractFaceFeature(byte[] data, int width, int height) {
        YTFaceTracker.Options options = new YTFaceTracker.Options();
        options.biggerFaceMode = false;
        options.maxFaceSize = 9999;
        options.minFaceSize = 50;

        YTFaceTracker.TrackedFace[] faceBoxes = mYTFaceTracker.detect(data, width, height, options);

        if (faceBoxes == null || faceBoxes.length == 0) {
            // TODO: 处理注册照不存在人脸的情况
            Log.w(TAG, "extractFaceFeature() no face found. called with: data = [" + data + "], width = [" + width + "], height = [" + height + "]");
            return null;
        } else if (faceBoxes.length > 1) {
            // TODO: 处理注册照存在多张人脸的异常情况
            Log.w(TAG, "extractFaceFeature() more than one face found. called with: data = [" + data + "], width = [" + width + "], height = [" + height + "]");
            return null;
        }

        float[] feat = new float[FACE_FEAT_LENGTH];
        int code = mYTFaceFeature.extract(faceBoxes[0].xy5Points, data, width, height, feat);
        if (code != 0) {
            // TODO: 处理提取特征失败
            Log.w(TAG, "extractFaceFeature() return code = " + code + ". called with: data = [" + data + "], width = [" + width + "], height = [" + height + "]");
            return null;
        }
        return feat;
    }
    ...
    public static void loadModels(AssetManager assetManager) {
        ...
        result = YTFaceLiveColor.globalInit(assetManager, "models/face-live-color-v123", "config.ini");
