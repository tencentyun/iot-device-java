...
import com.qcloud.iot_explorer.gateway.TXGatewaySubdevActionCallBack;
...
public class ProductAirconditioner {
    ...
    private ProductAirconditioner.reportPropertyPeriodically mReportThread = null;

    //light property
    public ConcurrentHashMap<String, Object> mProperty = null;

    public ProductAirconditioner(TXGatewayClient connection, Context context, String productId, String deviceName) {
        //初始化模板数据
        initTemplateData();
            ...
            if (Status.ERROR == status) {
                ...
                if(topic.substring(1,topic.length()-1).equals(mGatewaySubdev.mPropertyDownStreamTopic)) {
                    ...
                    if(null == mReportThread) {
                        mReportThread = new ProductAirconditioner.reportPropertyPeriodically(); //周期性上报
                        mReportThread.start();
                    }
                }
                TXLog.d(TAG,logInfo);
